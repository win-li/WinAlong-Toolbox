...
所有系统写操作都要求 root 权限并二次确认。Portainer 需要挂载 Docker socket，因此拥有较高的服务器管理权限；默认不公开管理端口，也不启用 Edge Agent 的 8000 端口。Docker、Portainer 和应用中心不会自动将用户加入 `docker` 用户组。恢复会覆盖指定应用的数据卷，因此额外要求输入 `RESTORE`，并自动创建恢复前快照。启用 UFW 或 BBR 额外要求输入 `ENABLE`。项目仍处于早期开发阶段（v0.9.0），请先在测试 VPS 验证，再用于重要环境。架构说明见 `docs/architecture.md`。

## 在线更新与插件

在线更新默认从 GitHub 私有仓库的 `main` 分支通过 SSH 拉取。使用 `sudo winalong` 更新时，工具会尝试使用 `SUDO_USER` 对应用户的 Ed25519 GitHub 密钥。更新只允许写入 `/opt/winalong-toolbox`，发现降级版本时会拒绝执行；安装前必须输入完整 `UPDATE`。
