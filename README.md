...
所有系统写操作都要求 root 权限并二次确认。Portainer 需要挂载 Docker socket，因此拥有较高的服务器管理权限；默认不公开管理端口，也不启用 Edge Agent 的 8000 端口。Docker、Portainer 和应用中心不会自动将用户加入 `docker` 用户组。恢复会覆盖指定应用的数据卷，因此额外要求输入 `RESTORE`，并自动创建恢复前快照。启用 UFW 或 BBR 额外要求输入 `ENABLE`。v1.0.0 已进入正式版阶段，但仍应先在测试 VPS 验证，再用于重要环境。架构说明见 `docs/architecture.md`，发布验收见 `docs/release.md`。
