...
- 备份健康状态、完整性校验、SHA-256 清单和创建前磁盘余量保护
- 默认每应用保留 7 个归档的手动清理预览与完整 `PURGE` 确认
- 固定允许清单的托管配置快照，不采集本地覆盖配置、SSH、密钥或日志
- 由诊断报告和备份摘要组成的权限 `600` 脱敏支持包
- `--backup-health`、`--backup-verify`、`--config-snapshot`、`--support-bundle` 入口
- 自动识别 Linux 发行版
- 统一消息输出与运行日志
- 安装、卸载和烟雾测试脚本
...
sudo ./toolbox.sh --backup-health
sudo ./toolbox.sh --backup-verify
sudo ./toolbox.sh --config-snapshot
sudo ./toolbox.sh --support-bundle
```
...
所有系统写操作都要求 root 权限并二次确认。Portainer 需要挂载 Docker socket，因此拥有较高的服务器管理权限；默认不公开管理端口，也不启用 Edge Agent 的 8000 端口。Docker、Portainer 和应用中心不会自动将用户加入 `docker` 用户组。恢复会覆盖指定应用的数据卷，因此额外要求输入 `RESTORE`，并自动创建恢复前快照。启用 UFW、BBR 或自动备份额外要求输入完整确认词。v1.4.0 延续正式版安全边界，仍应先在测试 VPS 验证，再用于重要环境。架构说明见 `docs/architecture.md`，日志与诊断说明见 `docs/diagnostics.md`，备份生命周期见 `docs/backup-lifecycle.md`，发布验收见 `docs/release.md`。
...
`sudo winalong --backup-health` 显示归档数量、最新时间、大小、完整性和磁盘余量；`--backup-verify` 校验所有标准归档并生成 SHA-256 清单。历史备份清理只在交互菜单提供，先预览，再要求输入完整 `PURGE`；不会由 Timer 自动执行。

`sudo winalong --config-snapshot` 创建固定允许清单的托管配置快照，不提供自动恢复。`sudo winalong --support-bundle` 生成权限为 `600` 的脱敏支持包，分享前仍应解压并人工检查。
